    ...
    <div class="col">
        ...
        <div class="card w-25">
            ...
            <div class="card-body">
                ...
                <div class="card-text">quantidade: <%=id.getInt("quantidade")%></div>
                <div class="card-text">pontos: <div id="valpontos<%=String.valueOf(i)%>"><%=iditem.getInt("pontos")*id.getInt("quantidade")%></div></div>
                <div class="card-text">vendedor:<%=id.getString("nomeu")%></div>
                <input class="bg-white text-primary form-control w-100" type="button" id="botaocard<%=String.valueOf(i)%>" value="comprar">
            </div>
            <% } %>
...
<div class="container d-flex" id="teusitems">
    ...
    <div class="row">
        ...
        <table class="table" id="tabelameusitens">
            <tr>
                <th>item:</th>
                <th>pontos:</th>
            </tr>
            <% for(int b=1;b<=utilizador.length();b++) {
                for (int c=1;c<=items.length();c++) {
                    JSONObject idutilizador = utilizador.getJSONObject(String.valueOf(b));
                    JSONObject iditemutilizador = idutilizador.getJSONObject("itemsu");
                    if (Objects.equals(idutilizador.getString("nome"),nomeSessao)) {
                    for (int d=1;d<=iditemutilizador.length();d++) {
                        JSONObject itemutilizador = iditemutilizador.getJSONObject(String.valueOf(d));
                        if (Objects.equals(itemutilizador.getString("id"), String.valueOf(c))) {
                        JSONObject nomeitem = items.getJSONObject(String.valueOf(c));%>
            <th>
                <td><%=nomeitem.getString("nome")%></td>
                <td><%=iditemutilizador.getString("quantidade")%></td>
            </th>
                    <%  }
                    }
                }
...
<div class="container w-50" id="adicionaritem">
    ...
    <form method="POST">
        ...
        <div id="texto"></div>
        <input type="button" name="submeter" id="submeter" value="submeter" class="btn-primary btn">
    </form>
...
<div class="container w-50" id="venderitem">
    ...
    <form method="POST">
        ...
        <div id="quantidadebox" class="d-flex">
            ...
            <div class="text-align-center" id="quanttextov"></div>
        </div>
        <div id="textovendas"></div>
        <input type="button" name="submeterv" id="submeterv" value="submeter" class="btn-primary btn">
    </form>
    <img src="acessorio.png" id="imagemv" class="w-25 h-25">
</div>
<div id="msg"></div>
<script>
    $(document).ready(function () {
        var imagem = document.getElementById("imagem")
        var imagemv = document.getElementById("imagemv")
                    ...
                    Object.entries(obj["vendas"]).forEach(([key, value]) => {
                        Object.entries(obj["items"]).forEach(([keyitem, valueitem]) => {
                            if (obj["items"][keyitem.toString()]["nome"] == $(this).find("option:selected").val()) {
                                ...
                                Object.entries(obj["utilizador"]).forEach(([keyu, valueu]) => {
                                    if (valueu["nome"] === $("#sessao").val()) {
                                        if (valueu["pontos"] > parseInt($("#quantidade-items").val())) {
                                            $("#quanttexto").text("quantidade insuficiente");
                                        }
                                    }
                                })
                            }
                ...
                fetch("dados.json")
                    ...
                    .then(obj => {
                        Object.entries(obj["vendas"]).forEach(([key, value]) => {
                            Object.entries(obj["items"]).forEach(([keyitem, valueitem]) => {
                                if (value["iditem"] === keyitem) {
                                    if ($("#pesquisa").val() == valueitem["nome"]) {
                                        $("#cardpesquisa").append("<div class='card'>")
                                        $("#cardpesquisa").append("<div class='card-body'>")
                                        $("#cardpesquisa").append("<img src='" + valueitem["nome"] + ".png' class='card-img-top' style=width:20%; height:40%;>")
                                        $("#cardpesquisa").append("<div class='card-text'>pontos:" + parseInt(valueitem["pontos"]) * parseInt(value["quantidade"]) + " </div>")
                                        $("#cardpesquisa").append(" <div class='card-text'>vendedor:" + value["nomeu"] + "</div>")
                                        $("#cardpesquisa").append("<input class='bg-white text-primary form-control w-100' type='button' id='botaocard' value='comprar'> </div> </div>")
                                    }
                                }
                            })
                        })
        ...
        fetch("dados.json")
            ...
            .then(obj => {
                Object.entries(obj["utilizador"]).forEach(([key, value]) => {
                    Object.entries(obj["utilizador"][key.toString()]["itemsu"]).forEach(([keyitemu, valueitemu]) => {
                        Object.entries(obj["items"]).forEach(([keyitem, valueitem]) => {
                            if (obj["utilizador"][key.toString()]["nome"] === $("#sessao").text()) {
                                ...
                                if (obj["utilizador"][key.toString()]["pontos"] > parseInt($("#quantidade-items").val())) {
                                    $("#quanttextov").text("quantidade insuficiente");
                                }
                            }
                        })
                    })
            ...
            imagemv.src = $(this).find("option:selected").val() + ".png";
        })
        <% for (int i=1;i<=vendas.length();i++) {
            for (int e=1;e <= items.length();e++) {
            JSONObject id = vendas.getJSONObject(String.valueOf(i));
            JSONObject iditem = items.getJSONObject(String.valueOf(e));
             if (Objects.equals(id.getString("iditem"), String.valueOf(e))) {
        %>
        $("#botaocard<%=String.valueOf(i)%>").click(function () {
        $.ajax({
            url: "/site_items_war_exploded/ComprarItem/",
            method: "POST",
            data:{ "nome":"<%=iditem.getString("nome")%>","quantidade":"<%=id.getInt("quantidade")%>","vendedor":"<%=id.getString("nomeu")%>"}
        })
        })
        <% } %>
        <% } %>
        ...
        $("#submeter").click(function() {
            $.ajax({
                url:"/site_items_war_exploded/adicionarItem/",
                method:"POST",
                data:{"nome":$("#itemselecionado").val(),"quantidade": $("#quantidade-items").val()}
            }).done(function(msg) {
                $("#msg").text(msg);
            })
        })
$("#submeterv").click(function () {
        $.ajax({
            url:"/site_items_war_exploded/VenderItem/",
            method:"POST",
            data:{"nome":$("#itemsavender").val(),"quantidade":$("#quantidade-items-venda").val()}
        }).done(function (msg) {
            $("#msg").text(msg);
        })
})
